/* 本文件大部分参考 https://github.com/ARMmbed/DAPLink/tree/master/source/daplink/drag-n-drop/virtual_fs.c
	从该文件修改而来，移植到RT Thread

	感兴趣的朋友可以参考 https://github.com/ARMmbed/DAPLink.git

	相对于Daplink，本文件仅实现 U盘设备 FAT16文件 到 FAL partition的写入映射， 而不对写入流进行解析和处理
 */
...
#define LOG_TAG "dap.vfs"
#define DBG_LVL DBG_INFO
#include <rtdbg.h>


static uint32_t read_zero(virtual_media_t* vm, uint32_t offset, uint8_t *data, uint32_t size);
static void write_none(virtual_media_t* vm, uint32_t offset, const uint8_t *data, uint32_t size);
static void write_fatv(virtual_media_t* vm, uint32_t offset, const uint8_t *data, uint32_t size);
static uint32_t read_mbr(virtual_media_t* vm, uint32_t offset, uint8_t *data, uint32_t size);
static uint32_t read_fat(virtual_media_t* vm, uint32_t offset, uint8_t *data, uint32_t size);
static uint32_t read_dir(virtual_media_t* vm, uint32_t offset, uint8_t *data, uint32_t size);
static void write_dir(virtual_media_t* vm, uint32_t offset, const uint8_t *data, uint32_t size);
...
static int filename_valid(const vfs_filename_t filename);
static int filename_character_valid(char character);

static check_allowed_file_cb_t check_allowed_file_cb = NULL;
static partition_writted_done_cb partition_writted_done = NULL;

const virtual_media_t virtual_media_tmpl[] = {
    /*  Read CB         Write CB        Region Size                 Region Name     */
    {   read_mbr,       write_none,     VFS_SECTOR_SIZE         },  /* MBR         0 */
    {   read_fat,       write_fatv,     0 /* Set at runtime */  },  /* FAT1         1*/
    {   read_fat,       write_fatv,     0 /* Set at runtime */  },  /* FAT2         66*/
    {   read_dir,       write_dir,      VFS_SECTOR_SIZE * 2     },  /* Root Dir     131 132*/
...
static virtual_media_t virtual_media[16];
static root_dir_t dir_current;
static uint8_t file_count;

static uint32_t virtual_media_idx;
static uint32_t fat_idx;
static uint32_t dir_idx;
static uint32_t data_start;


static uint32_t data_sector_base = 0;
...
static uint32_t read_zero(virtual_media_t* vm, uint32_t offset, uint8_t *data, uint32_t num_sectors)
{
	LOG_D("read_zero offset=%d num=%d",offset,num_sectors);
...
static void write_none(virtual_media_t* vm, uint32_t offset, const uint8_t *data, uint32_t size)
{
	LOG_D("write_none offset=%d num=%d",offset,size);
}
static void write_fatv(virtual_media_t* vm, uint32_t offset, const uint8_t *data, uint32_t size)
{
	LOG_D("write_fat offset=%d num=%d",offset,size);
	LOG_HEX("fat",16,(rt_uint8_t*)data,32);
	
}
static uint32_t read_mbr(virtual_media_t* vm, uint32_t sector_offset, uint8_t *data, uint32_t size)
{
	uint32_t read_size = sizeof(mbr_t);
    COMPILER_ASSERT(sizeof(mbr_t) <= VFS_SECTOR_SIZE);
	LOG_D("read_mbr offset=%d num=%d",sector_offset,size);
    if (sector_offset != 0) {
...
static uint32_t read_fat(virtual_media_t* vm, uint32_t sector_offset, uint8_t *data, uint32_t num_sectors)
{
	LOG_D("read_fat offset=%d num=%d",sector_offset,num_sectors);
...
static uint32_t read_dir(virtual_media_t* vm, uint32_t sector_offset, uint8_t *data, uint32_t num_sectors)
{
	LOG_D("read_dir offset=%d num=%d",sector_offset,num_sectors);
	 if ((sector_offset + num_sectors) * VFS_SECTOR_SIZE > sizeof(root_dir_t)) {
...
static void write_dir(virtual_media_t* vm, uint32_t sector_offset, const uint8_t *data, uint32_t num_sectors)
{
	LOG_D("write_dir offset=%d num=%d",sector_offset,num_sectors);
    ...
    for (; i < num_entries; i++) {
        ...
        // Changed
        file_change_cb_stub(new_entry[i].filename, VFS_FILE_CHANGED, (vfs_file_t)&old_entry[i], (vfs_file_t)&new_entry[i]);

        // Deleted
        if (0xe5 == (uint8_t)new_entry[i].filename[0]) {
            	file_change_cb_stub(old_entry[i].filename, VFS_FILE_DELETED, (vfs_file_t)&old_entry[i], (vfs_file_t)&new_entry[i]);
            continue;
        }

        // Created
        if (!same_name && filename_valid(new_entry[i].filename) ) {
            	file_change_cb_stub(new_entry[i].filename, VFS_FILE_CREATED, (vfs_file_t)&old_entry[i], (vfs_file_t)&new_entry[i]);
            continue;
        }
    }
...
static void vfs_file_set_attr(vfs_file_t file, vfs_file_attr_bit_t attr)
{
    FatDirectoryEntry_t *de = file;
    de->attributes = attr;
}
static uint32_t vfs_file_get_size(vfs_file_t file)
{
    FatDirectoryEntry_t *de = file;
    return de->filesize;
}
static vfs_sector_t vfs_file_get_start_sector(vfs_file_t file)
{
    FatDirectoryEntry_t *de = file;
...
static vfs_file_attr_bit_t vfs_file_get_attr(vfs_file_t file)
{
    FatDirectoryEntry_t *de = file;
...
static void vfs_file_set_size(vfs_file_t file, uint32_t filesize)
{
    FatDirectoryEntry_t *de = file;
    de->filesize = filesize;
	
	uint32_t cluster_size = mbr.bytes_per_sector * mbr.sectors_per_cluster;
    uint32_t clusters = (filesize + cluster_size - 1) / cluster_size;
	
	for(int i = 0;i< virtual_media_idx;i++)
	{
		if(strncasecmp(virtual_media[i].user_data,de->filename,strlen(virtual_media[i].user_data)) == 0)
		{
			virtual_media[i].length = clusters * mbr.bytes_per_sector * mbr.sectors_per_cluster;
			/* 修改写入sector 位置*/
			virtual_media[i].start_offset =  (last_written_sector - data_sector_base)/mbr.sectors_per_cluster*mbr.sectors_per_cluster + data_sector_base + mbr.sectors_per_cluster;
			LOG_D("sector offset = %d",virtual_media[i].start_offset );
		}
	}
}
vfs_file_t vfs_create_file(const vfs_filename_t filename, vfs_read_cb_t read_cb, vfs_write_cb_t write_cb, uint32_t len)
{
	...
	if(0 == filename_valid(filename))
	{
		return NULL;
	}
	
	LOG_D("Try to create File : %8s",filename);
	...
	if(write_cb != RT_NULL)
	{
		...
		if(check_allowed_file_cb != RT_NULL)
		{
			pname = check_allowed_file_cb(( char *)filename);
		}
		else
		{
			return RT_NULL;
		}
		if(pname == RT_NULL) return RT_NULL;
		LOG_D("Create File : %8s partition: %s",filename,pname);
	}
	...
	virtual_media[virtual_media_idx].user_data = (void*)pname;
    if (0 != read_cb) {
...
void vfs_read(uint32_t requested_sector, uint8_t *buf, uint32_t num_sectors)
{
    ...
    current_sector = 0;
	LOG_D("vfs_read <--- Rsecotor = %d",requested_sector);
    for (i = 0; i < ARRAY_SIZE(virtual_media); i++) {
		...
		if(virtual_media[i].start_offset != 0)
		 {
			 vm_start = current_sector = virtual_media[i].start_offset;
			 vm_end = vm_start + vm_sectors;
		 }
        ...
        if ((requested_sector >= vm_start) && (requested_sector < vm_end)) {
            ...
            virtual_media[i].read_cb(&virtual_media[i],sector_offset, buf, sectors_to_write);
...
void vfs_write(uint32_t requested_sector, const uint8_t *buf, uint32_t num_sectors)
{
    ...
    uint32_t current_sector;
    current_sector = 0;
	LOG_D("vfs_write ---> Rsecotor = %d",requested_sector);
	
	last_written_sector = requested_sector > last_written_sector?requested_sector:last_written_sector;
    for (i = 0; i < virtual_media_idx; i++) {
        ...
        if ((requested_sector >= vm_start) && (requested_sector < vm_end)) {
            ...
            sector_offset = requested_sector - current_sector;
            virtual_media[i].write_cb(&virtual_media[i],sector_offset, buf, sectors_to_read);
            // Update requested sector
...
static int is_written_done(const char* partition_name, uint32_t written_size)
{
	FatDirectoryEntry_t *de;
	for(int i = 1; i< dir_idx; i++)
	{
		de = &dir_current.f[i];
		if(strncasecmp(de->filename, partition_name,strlen(partition_name)) == 0)
		{
			if(written_size >= de->filesize)
			{
				if(partition_writted_done)
				{
					partition_writted_done(partition_name,de->filesize);
				}
				
				if(strncmp(de->filename + 8,"RST",3) == 0)
				{
					LOG_D("!!!!!! Need Reset !!!!!");
					rt_hw_cpu_reset();
				}
...
static uint32_t partition_read_cb(virtual_media_t* vm, uint32_t sector_offset, uint8_t *data, uint32_t num_sectors)
{

	const char* partition_name = (const char*)vm->user_data;

	const struct fal_partition * partition = fal_partition_find(partition_name);
	LOG_D("partiton_read pname=%s offset=%d num=%d",partition_name,sector_offset,num_sectors);
	const struct fal_partition * p = fal_partition_find(partition_name);
	fal_partition_read(p,sector_offset * VFS_SECTOR_SIZE,data,VFS_SECTOR_SIZE);
	return VFS_SECTOR_SIZE;
}
static void partiton_wirte_cb(virtual_media_t* vm, uint32_t sector_offset, const uint8_t *data, uint32_t num_sectors)
{
	const char* partition_name = (const char*)vm->user_data;
	const struct fal_partition * partition = fal_partition_find(partition_name);
	
	LOG_D("partiton_wirte pname=%s offset=%d num=%d",partition_name,sector_offset,num_sectors);
	const struct fal_partition * p = fal_partition_find(partition_name);

	const struct fal_flash_dev *flash_dev = fal_flash_device_find(p->flash_name);
	
	char * buffer = (char *)rt_malloc(flash_dev->blk_size);
	if(buffer == RT_NULL)
	{
		LOG_E("no memory");
		return;
	}
	//假设flash 的擦除块 都是512字节的整数倍
	uint32_t addr = sector_offset * VFS_SECTOR_SIZE/flash_dev->blk_size*flash_dev->blk_size;
	
	LOG_D("partition flash blk_size = %d addr = %x", flash_dev->blk_size, addr);
	fal_partition_read(p,addr ,(uint8_t*)buffer,flash_dev->blk_size);
	fal_partition_erase(p,addr,VFS_SECTOR_SIZE);

	memcpy(buffer + (sector_offset * VFS_SECTOR_SIZE) % flash_dev->blk_size, data,VFS_SECTOR_SIZE);
	fal_partition_write(p,addr,(uint8_t*)buffer,flash_dev->blk_size);

	rt_free(buffer);
	
	is_written_done(partition_name,(sector_offset+1)*VFS_SECTOR_SIZE);
}
...
static void file_change_cb_stub(const vfs_filename_t filename, vfs_file_change_t change,
                                vfs_file_t file, vfs_file_t new_file_data)
{
	LOG_D("file_change_cb_stub filename=%11s change=%d",filename,change);
	FatDirectoryEntry_t * fatEntry = (FatDirectoryEntry_t *)new_file_data;
	
	LOG_D("attr: %08x",fatEntry->attributes);
	LOG_D("size: %d",fatEntry->filesize);
	LOG_D("cluster_H: %d",fatEntry->first_cluster_high_16);
	LOG_D("cluster_L: %d",fatEntry->first_cluster_low_16);
	if(change == VFS_FILE_CREATED)
	{
		if(fatEntry->attributes & VFS_FILE_ATTR_ARCHIVE)
		{
			vfs_create_file(filename,partition_read_cb,partiton_wirte_cb, fatEntry->filesize);
		}
	
	}
	else if(change == VFS_FILE_CHANGED)
	{
		if(fatEntry->filesize != 0)
		{
			vfs_file_set_size(file,fatEntry->filesize);
		}
...
void vfs_init(const vfs_filename_t drive_name, uint32_t disk_size, check_allowed_file_cb_t cb, partition_writted_done_cb written_done_fn)
{
	...
	partition_writted_done = written_done_fn;
    // Clear everything
	...
	data_sector_base = mbr.logical_sectors_per_fat * 2 + 1/*mbr*/ + 2/*rootdir*/;
    // Initialize indexes
    virtual_media_idx = MEDIA_IDX_COUNT; //指到 实际存储位置
    data_start = 0;

    for (i = 0; i < ARRAY_SIZE(virtual_media_tmpl); i++) {  //实际位置
...
static int filename_valid(const vfs_filename_t filename)
{
	...
	/* 对win10自动写入的文件过滤 */
	for (i = 8;i < sizeof(vfs_filename_t); i++) {
